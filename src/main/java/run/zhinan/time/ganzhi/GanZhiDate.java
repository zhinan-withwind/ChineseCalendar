...
public class GanZhiDate extends BaseDate implements TemporalAccessor {
    ...
    public static GanZhiDate of(LocalDate date) {
        ...
        int currentYear = date.getYear() - (date.isBefore(springDay) ? 1 : 0);
        GanZhi year  = GanZhi.toGanZhi(currentYear);

        // 第二步：计算月份干支
        GanZhi month = GanZhi.toGanZhi(currentYear, SolarTerm.getLastMajorSolarTerm(date.atTime(0, 0)).getDateTime().getMonthValue());
